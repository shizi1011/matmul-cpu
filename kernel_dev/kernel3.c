...
void copy_pad_blockA(const float *A, float *blockA_padded, int m, int M,
                     int K) {
  for (int k = 0; k < K; ++k) {

    for (int i = 0; i < m; ++i) {
      blockA_padded[k * 16 + i] = A[k * M + i];
    }
    for (int i = m; i < 16; ++i) {
      blockA_padded[k * 16 + i] = 0.0;
    }
  }
}
void copy_pad_blockB(const float *B, float *blockB_padded, int n, int K,
                     int N) {

  for (int i = 0; i < 6; ++i) {
    for (int k = 0; k < K; ++k) {
      if (i < n)
        blockB_padded[k + i * K] = B[k + i * K];
      else
        blockB_padded[k + i * K] = 0.0;
    }
  }
}

void kernel_16x6_pad(const float *A, const float *B, float *C, int M, int N,
                     int m, int n, int K, int blockA_1d) {
...
void matmul_kernel(const float *A, const float *B, float *C, int M, int N,
  ...
  for (int i = 0; i < M; i += 16) {
    ...
    for (int j = 0; j < N; j += 6) {
      int n = min(6, N - j);
      const float *blockB = &B[j * K];
      if (n != 6) {
        copy_pad_blockB(&B[j * K], blockB_buffer, n, K, N);
        blockB = blockB_buffer;
      }
      kernel_16x6_pad(blockA, blockB, &C[j * M + i], M, N, m, n, K, blockA_1d);
    }
  }
}
